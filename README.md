...
This branch is the repository state at the end of [Class 6: LangGraph4j Integration](https://graphqlguy.com/docs/tutorial-ai-agents/langgraph-integration).

**What Class 6 adds:** `LangGraphConcierge` and `ConciergeState`, the loop drawn as a state machine with a checkpoint before every tool step, and the `graph-agent` command.

**Following along:** start from `agents_class_5`, work through the lesson, then compare your result with this branch. The next class with code is `agents_class_7`.
