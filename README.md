...
This branch is the repository state at the end of [Class 8: Structured Output via Fragments](https://graphqlguy.com/docs/tutorial-ai-agents/structured-output-via-fragments).

**What Class 8 adds:** `RecommendCommand` and `RecommendOneCallCommand`, which turn a tool-calling run into a typed `MovieRecommendation` list.

**Following along:** start from `agents_class_7`, work through the lesson, then compare your result with this branch. The next class with code is `agents_class_9`.
