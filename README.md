...
This branch is the repository state at the end of [Class 10: Cost Accounting per Agent Run](https://graphqlguy.com/docs/tutorial-ai-agents/cost-accounting-per-agent-run).

**What Class 10 adds:** `CostMeter`, which prices a run and reports it through Micrometer, and the receipt the `agent` command now prints.

**Following along:** start from `agents_class_9`, work through the lesson, then compare your result with this branch. The next class with code is `agents_class_11`.
