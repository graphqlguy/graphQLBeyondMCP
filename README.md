...
This branch is the repository state at the end of [Class 7: Spring AI Integration](https://graphqlguy.com/docs/tutorial-ai-agents/spring-ai-integration).

**What Class 7 adds:** the `spring-agent` and `spring-stream` commands on Spring AI's advisor-owned loop, and the tool-search work: `ToolSearchCommand` plus the two benchmark commands behind the measurements in the lesson.

**Following along:** start from `agents_class_6`, work through the lesson, then compare your result with this branch. The next class with code is `agents_class_8`.
