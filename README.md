...
This branch is the repository state at the end of [Class 5: LangChain4j Integration](https://graphqlguy.com/docs/tutorial-ai-agents/langchain4j-integration).

**What Class 5 adds:** `LangChainSchemaMapper` and `LangChainToolBridge`, which carry the same catalog into LangChain4j's `AiServices`, plus the `lc4j-agent` command.

**Following along:** start from `agents_class_4`, work through the lesson, then compare your result with this branch. The next class with code is `agents_class_6`.
