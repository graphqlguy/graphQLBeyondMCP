...
This branch is the repository state at the end of [Class 2: Auto-Generating Tool Definitions from a Schema](https://graphqlguy.com/docs/tutorial-ai-agents/auto-generating-tool-definitions).

**What Class 2 adds:** `ToolCatalogGenerator` and the pieces it needs: `AllowList`, `JsonSchemaMapper`, `OperationTool`, the committed schema snapshot, and the `catalog` and `call` commands that print a catalog and execute one tool.

**Following along:** start from `main`, work through the lesson, then compare your result with this branch. The next class with code is `agents_class_4`.
