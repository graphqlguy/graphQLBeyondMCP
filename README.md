...
This branch is the repository state at the end of [Class 11: Testing Agent-Driven Queries](https://graphqlguy.com/docs/tutorial-ai-agents/testing-agent-driven-queries).

**What Class 11 adds:** the test suite: the JSON Schema rules pinned as facts, the catalog held against a committed golden copy, the callback's safety behaviors asserted against an HTTP stub, and a tagged scenario test that runs the real loop.

**Following along:** start from `agents_class_10`, work through the lesson, then compare your result with this branch. This is the last branch; [Class 12](https://graphqlguy.com/docs/tutorial-ai-agents/code-execution-alternative) closes the course without adding code.
